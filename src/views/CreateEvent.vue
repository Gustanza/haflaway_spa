        ...
        <div class="ce-section">
          ...
          >
            ...
            <div v-else class="ce-thumb-placeholder">
              ...
              <p class="ce-thumb-hint">Click or drag an image here</p>
          ...
          <span v-if="errors.thumb" class="ce-field-error">{{ errors.thumb }}</span>
        </div>
        ...
        <div class="ce-modal">
          ...
          <div class="ce-modal-list">
            ...
            <button
              ...
              type="button"
              class="ce-modal-item"
              :class="{ 'ce-modal-item--active': form.categoryId === cat.id }"
        ...
        <div class="ce-modal">
          ...
          <div class="ce-modal-list">
            ...
            <button
              ...
              type="button"
              class="ce-modal-item"
              :class="{ 'ce-modal-item--active': form.eventPlanId === plan.id }"
...
function validate() {
  const e = {}
  if (!thumbFile.value) e.thumb = 'Please upload a thumbnail image.'
...
async function handleSubmit() {
  ...
  try {
    ...
    // Atomic write: event doc + default checkpoint
    const batch = writeBatch(db)
    batch.set(eventRef, eventData)
    const checkpointRef = doc(collection(db, 'events', eventRef.id, 'checkpoints'))
...
onMounted(async () => {
  ...
  try {
    const [catSnap, planSnap] = await Promise.all([
...
.ce-textarea::placeholder { color: #B5B0A8; }
.ce-input:focus,
.ce-textarea:focus {
